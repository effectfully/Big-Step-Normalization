...
mutual
  data ⟦_⟧_⇓_ : ∀ {Γ Δ σ} -> Γ ⊢ σ -> ⟦ Γ ↦ Δ ⟧ -> ⟦ Δ ⊢ⁿᶠ σ ⟧ -> Set where
    ø⇓      : ∀ {Γ Δ σ} {s : ⟦ Δ ⊢ⁿᶠ σ ⟧} {ρ : ⟦ Γ ↦ Δ ⟧}
            -> ⟦ ø ⟧ (ρ ▻ᵉⁿᵛ s) ⇓ s
    ƛ⇓_     : ∀ {Γ Δ σ τ} {b : Γ ▻ σ ⊢ τ} {ρ : ⟦ Γ ↦ Δ ⟧}
            -> ⟦ ƛ b ⟧ ρ ⇓ ƛˢᵉᵐ b ρ
    _∙⟨_⟩⇓_ : ∀ {Γ Δ σ τ} {f : Γ ⊢ σ ⇒ τ} {x : Γ ⊢ σ} {ρ : ⟦ Γ ↦ Δ ⟧} {fʳ xʳ fxʳ}
            -> ⟦ f ⟧ ρ ⇓ fʳ -> fʳ $ xʳ ⇓ fxʳ -> ⟦ x ⟧ ρ ⇓ xʳ -> ⟦ f ∙ x ⟧ ρ ⇓ fxʳ
  ...
  data ⟦_⟧ˢᵘᵇ_⇓_ : ∀ {Γ Δ Θ} -> Γ ↦ Δ -> ⟦ Δ ↦ Θ ⟧ -> ⟦ Γ ↦ Θ ⟧ -> Set where
    ...
    _▻ˢᵘᵇ⇓_ : ∀ {Γ Δ Θ σ} {x : Δ ⊢ σ} {ψ : Γ ↦ Δ} {ρ : ⟦ Δ ↦ Θ ⟧} {xʳ ρʳ}
            -> ⟦ ψ ⟧ˢᵘᵇ ρ ⇓ ρʳ -> ⟦ x ⟧ ρ ⇓ xʳ -> ⟦ ψ ⟧ˢᵘᵇ ρ ⇓ ρʳ
    ↑⇓      : ∀ {Γ Δ σ} {xˢ : ⟦ Δ ⊢ⁿᶠ σ ⟧} {ρ : ⟦ Γ ↦ Δ ⟧}
            -> ⟦ ↑ ⟧ˢᵘᵇ ρ ▻ᵉⁿᵛ xˢ ⇓ ρ

  data _$_⇓_ : ∀ {Γ σ τ} -> ⟦ Γ ⊢ⁿᶠ σ ⇒ τ ⟧ -> ⟦ Γ ⊢ⁿᶠ σ ⟧ -> ⟦ Γ ⊢ⁿᶠ τ ⟧ -> Set where
    ne$⇓ : ∀ {Γ σ τ} {f : ⟦ Γ ⊢ⁿᵉ σ ⇒ τ ⟧} {x : ⟦ Γ ⊢ⁿᶠ σ ⟧}
         -> neˢᵉᵐ f $ x ⇓ neˢᵉᵐ (f ∙ x)
    ƛ$⇓_ : ∀ {Γ Δ σ τ} {b : Γ ▻ σ ⊢ τ} {ρ : ⟦ Γ ↦ Δ ⟧} {xˢ : ⟦ Δ ⊢ⁿᶠ σ ⟧} {bʳ}
         -> ⟦ b ⟧ ρ ▻ᵉⁿᵛ xˢ ⇓ bʳ -> ƛˢᵉᵐ b ρ $ xˢ ⇓ bʳ

mutual
  data Quoteⁿᶠ_⇓_ : ∀ {Γ σ} -> ⟦ Γ ⊢ⁿᶠ σ ⟧ -> Γ ⊢ⁿᶠ σ -> Set where
    ne⇓ : ∀ {Γ} {n : ⟦ Γ ⊢ⁿᵉ ⋆ ⟧} {nʳ}
        -> Quoteⁿᵉ n ⇓ nʳ -> Quoteⁿᶠ (neˢᵉᵐ n) ⇓ neⁿᶠ nʳ
    ƛ⇓_ : ∀ {Γ σ τ} {f : ⟦ Γ ⊢ⁿᶠ σ ⇒ τ ⟧} {bˢʳ bʳʳ}
        -> pop⟦⟧ⁿᶠ f $ neˢᵉᵐ (var vz) ⇓ bˢʳ -> Quoteⁿᶠ bˢʳ ⇓ bʳʳ -> Quoteⁿᶠ f ⇓ ƛⁿᶠ bʳʳ
